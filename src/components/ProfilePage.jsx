...
const initialUser = {
  ...
  username:  currentUser.firstName && currentUser.lastName
    ? (currentUser.firstName + currentUser.lastName).toLowerCase().replace(/\s/g, '') + '_qrib'
    : 'neighbor_qrib',
  neighborhood: currentUser.neighborhood,
  city:         currentUser.city,
  avatar:       currentUser.avatar,
  bio: `Hi! I'm ${currentUser.name}, living in ${currentUser.neighborhood}. Love connecting with neighbors and helping out 🤝`,
...
function AnimatedNumber({ value }) {
  ...
  useEffect(() => {
    let current = 0
    const steps = 40, increment = value / steps
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${color}${hovered ? '25' : '15'}` }}>
          <Icon size={14} style={{ color }} />
        </div>
        <span className={`text-xs font-medium ${dark ? 'text-purple-300/50' : 'text-gray-400'}`}>{label}</span>
      </div>
      <p className={`text-2xl font-black ${dark ? 'text-purple-50' : 'text-gray-900'}`}><AnimatedNumber value={value} /></p>
    </div>
  )
}
...
function SkillTag({ label, color, onRemove, editing }) {
  return (
    <span className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full border transition-all duration-150 hover:scale-105"
      ...
      {label}
      {editing && <button onClick={() => onRemove(label)} className="hover:opacity-60 ml-0.5"><FiX size={11} /></button>}
    </span>
  )
}
...
function CircularProgress({ current, max, color, size = 80, strokeWidth = 7 }) {
  ...
  useEffect(() => { const t = setTimeout(() => setAnimated(pct), 80); return () => clearTimeout(t) }, [pct])
  return (
    <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
      <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={dark ? 'rgba(255,255,255,0.08)' : '#f3f4f6'} strokeWidth={strokeWidth} />
      <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={color} strokeWidth={strokeWidth}
        strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={circumference * (1 - animated)}
...
function RewardCard({ reward, user, claimed, onClaim }) {
  ...
  return (
    <div className="rounded-3xl border shadow-sm p-5 flex items-center gap-5"
      style={{ backgroundColor: dark ? '#1a0a2e' : 'white', borderColor: complete && !claimed ? `${reward.color}40` : (dark ? 'rgba(255,255,255,0.06)' : '#f3f4f6') }}>
      <div className="relative shrink-0 flex items-center justify-center" style={{ width: 80, height: 80 }}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5">
          ...
          <span className="text-[11px] font-black" style={{ color: claimed ? dimColor : reward.color }}>{claimed ? '✓' : `${pct}%`}</span>
        </div>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <p className={`font-black text-sm ${dark ? 'text-purple-50' : 'text-gray-900'}`}>{reward.label}</p>
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${reward.color}15`, color: reward.color }}>+{reward.xpReward} XP</span>
        </div>
      ...
      <div className="shrink-0">
        {claimed ? (
          <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${dark ? 'bg-white/8' : 'bg-gray-100'}`}><FiCheck size={15} style={{ color: dimColor }} /></div>
        ) : complete ? (
          <button onClick={() => onClaim(reward)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-white text-xs font-black hover:opacity-90 active:scale-95 transition-all"
            style={{ background: `linear-gradient(135deg,${reward.color},${reward.color}cc)` }}><FaCoins size={11} /> Claim</button>
        ) : (
          <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ backgroundColor: `${reward.color}10` }}><FiZap size={14} style={{ color: `${reward.color}60` }} /></div>
        )}
      </div>
    </div>
  )
...
export default function ProfilePage({ onBack }) {
  ...
  function addSkill(type) {
    const val = newSkill[type].trim(); if (!val) return
  ...
  function handleAvatarUpload(e) {
    ...
    reader.onload = (ev) => {
      ...
      try {
        const stored = JSON.parse(localStorage.getItem('qriblikUser') || '{}')
        stored.avatar = dataUrl
        localStorage.setItem('qriblikUser', JSON.stringify(stored))
      } catch {}
    }
  ...
  function RedeemModal({ xp, onClose, onRedeem, redeemedIds }) {
    ...
    function handleFinalRedeem() { onRedeem(confirming); setSuccess(confirming); setConfirming(null); setTimeout(() => setSuccess(null), 3000) }
    return (
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4"
        style={{ backgroundColor: 'rgba(0,0,0,0.55)', backdropFilter: 'blur(4px)' }}
        onClick={e => e.target === e.currentTarget && onClose()}>
        <div className="rounded-3xl w-full max-w-md shadow-2xl overflow-hidden"
          style={{ maxHeight: '85vh', backgroundColor: dark ? '#1a0a2e' : 'white', border: dark ? '1px solid rgba(139,63,222,0.2)' : 'none' }}>
          <div className={`px-6 pt-6 pb-4 flex items-center justify-between border-b ${dark ? 'border-white/8' : 'border-gray-100'}`}>
            <div>
              <h2 className={`text-base font-black flex items-center gap-2 ${dark ? 'text-purple-50' : 'text-gray-900'}`}><FiGift size={16} style={{ color: '#C837AB' }} /> Spend your XP</h2>
              <p className={`text-xs mt-0.5 ${dark ? 'text-purple-300/50' : 'text-gray-400'}`}>You have <span className="font-black" style={{ color: '#C837AB' }}>{xp} XP</span> available</p>
            </div>
            <button onClick={onClose} className={`w-8 h-8 rounded-xl flex items-center justify-center ${dark ? 'bg-white/8 text-purple-300' : 'bg-gray-100 text-gray-500'}`}><FiX size={15} /></button>
          </div>
          {success && (
            <div className="mx-4 mt-4 px-4 py-3 rounded-2xl text-sm font-semibold flex items-center gap-2" style={{ backgroundColor: '#1DB95415', color: '#1DB954', border: '1px solid #1DB95430' }}>
              <FiCheck size={15} /><span><strong>{success.label} ({success.period})</strong> code sent to your email!</span>
            </div>
          )}
          {confirming && (
            <div className="mx-4 mt-4 p-4 rounded-2xl border-2 flex flex-col gap-3" style={{ borderColor: `${confirming.color}40`, backgroundColor: dark ? `${confirming.color}12` : `${confirming.color}08` }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-xs ${dark ? 'text-purple-300/50' : 'text-gray-500'}`}>Deducts <strong style={{ color: confirming.color }}>{confirming.cost} XP</strong> from your balance</p>
                </div>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setConfirming(null)} className={`flex-1 py-2 rounded-xl border text-sm font-semibold ${dark ? 'border-white/10 text-purple-300' : 'border-gray-200 text-gray-500'}`}>Cancel</button>
                <button onClick={handleFinalRedeem} className="flex-1 py-2 rounded-xl text-white text-sm font-black" style={{ backgroundColor: confirming.color }}>Confirm Redeem</button>
              </div>
            </div>
          )}
          <div className="overflow-y-auto px-4 py-4 flex flex-col gap-3" style={{ maxHeight: '55vh' }}>
            {REDEEM_OPTIONS.map(option => {
              ...
              return (
                <div key={option.id} className="flex items-center gap-4 p-4 rounded-2xl border"
                  style={{ borderColor: alreadyRedeemed ? (dark ? 'rgba(255,255,255,0.05)' : '#e5e7eb') : canAfford ? `${option.color}30` : (dark ? 'rgba(255,255,255,0.04)' : '#f3f4f6'), backgroundColor: alreadyRedeemed ? (dark ? 'rgba(255,255,255,0.02)' : '#f9fafb') : (dark ? `${option.color}10` : option.bg), opacity: alreadyRedeemed ? 0.55 : 1 }}>
                  <div className="w-11 h-11 rounded-2xl flex items-center justify-center shrink-0" style={{ backgroundColor: `${option.color}20` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      {!canAfford && !alreadyRedeemed && <span className={`text-[10px] ${dark ? 'text-purple-300/40' : 'text-gray-400'}`}>· need {option.cost - xp} more</span>}
                    </div>
                  </div>
                  {alreadyRedeemed ? (
                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${dark ? 'bg-white/8' : 'bg-gray-100'}`}><FiCheck size={13} /></div>
                  ) : (
                    <button onClick={() => handleConfirm(option)} disabled={!canAfford}
                      ...
                      style={{ backgroundColor: canAfford ? option.color : (dark ? 'rgba(255,255,255,0.06)' : '#f3f4f6'), color: canAfford ? 'white' : (dark ? 'rgba(167,139,250,0.35)' : '#9ca3af'), cursor: canAfford ? 'pointer' : 'not-allowed' }}>
                      Redeem
                    </button>
                  )}
                </div>
  ...
  return (
    <div className={`flex-1 min-h-screen overflow-y-auto transition-colors duration-500 ${dark ? 'bg-[#0f0a1e]' : 'bg-gray-50'}`}>
      {showRedeemModal && <RedeemModal xp={xp} onClose={() => setShowRedeemModal(false)} onRedeem={handleRedeem} redeemedIds={redeemedIds} />}

      {/* ── TOP BAR — sticky, works on all screen sizes ── */}
      <div
        className={`sticky top-0 z-40 border-b px-4 md:px-6 py-3 flex items-center gap-3 transition-colors duration-500 ${
          dark ? 'bg-[#0d0719] border-white/5' : 'bg-white border-gray-100'
        }`}
      >
        {/* Back button — always visible, left side */}
        <button
          onClick={onBack}
          className={`flex items-center gap-2 text-sm font-semibold transition-colors group shrink-0 ${
            dark ? 'text-purple-300/60 hover:text-purple-200' : 'text-gray-500 hover:text-gray-800'
          }`}
        >
        ...
        {/* XP badge */}
        <div
          className="flex items-center gap-1.5 px-3 h-9 rounded-2xl border font-black text-sm shrink-0"
          style={{
        ...
        >
          <FaCoins size={12} /> <span>{xp} XP</span>
        </div>
        ...
        {editing ? (
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              onClick={handleSave}
              className="flex items-center gap-1.5 px-3 py-2 text-sm rounded-xl text-white font-semibold hover:opacity-90 active:scale-95 transition-all"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 text-sm rounded-xl border font-medium transition-colors shrink-0 ${
              dark ? 'border-white/10 text-purple-300 hover:bg-white/5' : 'border-gray-200 text-gray-600 hover:bg-gray-50'
            }`}
          >
            <FiEdit2 size={13} />
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-6 flex flex-col gap-6">
        ...
        <div className={`rounded-3xl border shadow-sm overflow-hidden transition-colors duration-500 ${card}`}>
          <div className="relative h-24 md:h-28 w-full" style={{ background: 'var(--gradient-qriblik)' }}>
            {editing && (
          ...
          <div className="px-5 md:px-6 pb-6">
            <div className="flex items-end justify-between -mt-10 md:-mt-12 mb-5">
              <div className="relative group inline-block">
                <div className={`p-[3px] rounded-full shadow-lg inline-block ${dark ? 'bg-[#150d27]' : 'bg-white'}`}>
                  <div className="p-[2px] rounded-full" style={{ background: 'var(--gradient-qriblik)' }}>
                    <img
                      src={editing ? draft.avatar : user.avatar}
                      alt={user.name}
                      className="w-16 h-16 md:w-20 md:h-20 rounded-full object-cover block bg-gray-400"
                    />
                  </div>
                </div>
                {editing && (
              ...
              {user.verified && (
                <span className="flex items-center gap-1.5 text-xs font-bold px-2.5 py-1.5 rounded-full text-white mb-1" style={{ background: 'var(--gradient-qriblik)' }}>
            ...
            {editing ? (
              <div className="flex flex-col gap-3">
                <input
                  ...
                  style={{ borderColor: '#8B3FDE' }} value={draft.name}
                ...
                <textarea
                  ...
                  style={{ borderColor: dark ? 'rgba(139,63,222,0.3)' : '#8B3FDE44' }} rows={2} maxLength={160}
                ...
                <div className="grid grid-cols-2 gap-3">
                  {['neighborhood', 'city'].map(field => (
                    <input key={field}
                      className={`text-sm border rounded-xl px-3 py-2 outline-none transition-colors ${dark ? 'bg-white/5 text-purple-200 border-white/10' : 'text-gray-600 border-gray-200'}`}
              ...
              <>
                ...
                <div className={`flex items-center gap-4 mt-3 text-xs flex-wrap ${dark ? 'text-purple-300/40' : 'text-gray-400'}`}>
                  <span className="flex items-center gap-1"><FiMapPin size={11} style={{ color: '#C837AB' }} />{user.neighborhood}, {user.city}</span>
                ...
                <div className="flex items-center gap-2 md:gap-3 mt-4 flex-wrap">
                  {[
                    ...
                    <button key={item.label} className="flex flex-col items-center px-3 md:px-4 py-2 rounded-2xl hover:scale-105 transition-all duration-150"
          ...
          {tabs.map(tab => {
            ...
            return (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                ...
                style={{
                  backgroundColor: isActive ? (dark ? '#1a0a2e' : 'white') : 'transparent',
                  color: isActive ? '#8B3FDE' : (dark ? 'rgba(167,139,250,0.4)' : '#9ca3af'),
                  boxShadow: isActive ? `0 1px 6px rgba(139,63,222,${dark ? '0.25' : '0.12'})` : 'none',
                }}>
                <tab.icon size={14} />
        ...
        {activeTab === 'reword' && (
          <div className="flex flex-col gap-4">
            <div className={`rounded-3xl border shadow-sm p-5 flex items-center justify-between ${card}`}>
              <div>
              ...
              <div className="flex items-center gap-2 px-4 py-2 rounded-2xl text-white font-black" style={{ background: 'var(--gradient-qriblik)' }}><FaCoins size={14} /> {xp} XP</div>
            </div>
            <button onClick={() => setShowRedeemModal(true)} disabled={xp === 0}
              ...
              style={{ background: xp > 0 ? 'var(--gradient-qriblik)' : undefined, backgroundColor: xp === 0 ? (dark ? 'rgba(255,255,255,0.05)' : '#f3f4f6') : undefined, color: xp > 0 ? 'white' : (dark ? 'rgba(167,139,250,0.3)' : '#9ca3af'), cursor: xp === 0 ? 'not-allowed' : 'pointer' }}>
              <FiGift size={15} />{xp === 0 ? 'Earn XP to unlock rewards' : `Spend ${xp} XP on rewards`}
            </button>
            <div className="flex flex-col gap-3">
          ...
          <>
            <div className={`rounded-3xl border shadow-sm p-6 ${card}`}>
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-2xl flex items-center justify-center" style={{ background: 'var(--gradient-qriblik)' }}><badgeMeta.icon size={20} className="text-white" /></div>
                  <div>
                ...
                <span className="text-3xl font-black" style={{ background: 'var(--gradient-qriblik)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Lv.{user.helpSystem.level}</span>
              </div>
              <div className={`h-3 rounded-full overflow-hidden ${dark ? 'bg-white/8' : 'bg-gray-100'}`}>
                <div className="h-full rounded-full transition-all duration-1000 relative overflow-hidden" style={{ width: `${levelPercent}%`, background: 'var(--gradient-qriblik)' }}>
              ...
              <div className="grid grid-cols-3 gap-2 md:gap-3 mt-5">
                {[
                  ...
                  <div key={item.label} className="text-center rounded-2xl py-3 px-1 md:px-2 hover:scale-105 transition-all duration-150 cursor-default"
                    style={{ backgroundColor: `${item.color}0d`, border: `1px solid ${item.color}20` }}>
                    <item.icon size={15} style={{ color: item.color }} className="mx-auto mb-1" />
                    <p className="text-lg md:text-xl font-black" style={{ color: item.color }}><AnimatedNumber value={item.value} /></p>
                    <p className={`text-[9px] md:text-[10px] mt-0.5 leading-tight ${dark ? 'text-purple-300/40' : 'text-gray-400'}`}>{item.label}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className={`rounded-3xl border shadow-sm p-6 ${card}`}>
              <h2 className={`text-sm font-black mb-4 ${dark ? 'text-purple-50' : 'text-gray-900'}`}>{editing ? 'Choose your categories' : 'Favorite Categories'}</h2>
              <div className="flex flex-wrap gap-2">
                {ALL_CATEGORIES.map(cat => {
                  const isActive = displayUser.favoriteCategories.includes(cat.label)
                  return (
                    <button key={cat.label} onClick={() => editing && toggleCategory(cat.label)} disabled={!editing}
                      className="flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-semibold transition-all duration-150"
                      style={{ color: isActive ? cat.color : (dark ? 'rgba(167,139,250,0.35)' : '#9ca3af'), borderColor: isActive ? `${cat.color}50` : (dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb'), backgroundColor: isActive ? `${cat.color}0d` : 'transparent', cursor: editing ? 'pointer' : 'default' }}>
                      <cat.icon size={14} />{cat.label}{editing && isActive && <FiCheck size={12} />}
                    </button>
                  )
                })}
              </div>
            ...
            <div className={`rounded-3xl border shadow-sm p-6 ${card}`}>
              ...
              <div className="flex items-center gap-5">
                <div className="w-16 h-16 rounded-2xl flex flex-col items-center justify-center text-white shrink-0" style={{ background: 'var(--gradient-qriblik)' }}>
                ...
                <div className="flex-1">
                  <div className="flex gap-1 mb-2">
                    {Array.from({ length: 5 }).map((_, i) => (
                      <FiStar key={i} size={20} onMouseEnter={() => setHoveredStar(i+1)} onMouseLeave={() => setHoveredStar(null)}
                        className="cursor-pointer transition-transform hover:scale-125 duration-100"
                        style={{ color: i < (hoveredStar ?? Math.round(user.helpSystem.trading.rating)) ? '#FF6B35' : (dark ? 'rgba(255,255,255,0.1)' : '#e5e7eb'), fill: i < (hoveredStar ?? Math.round(user.helpSystem.trading.rating)) ? '#FF6B35' : 'none' }} />
                    ))}
                  </div>
              ...
              {[
                ...
                <div key={row.label} className="mb-4 last:mb-0">
                  ...
                  <div className={`h-2.5 rounded-full overflow-hidden ${dark ? 'bg-white/8' : 'bg-gray-100'}`}>
                    <div className="h-full rounded-full transition-all duration-700" style={{ width: `${Math.min(row.pts/row.max*100,100)}%`, backgroundColor: row.color }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'skills' && (
          <div className="flex flex-col gap-4">
            {[
              ...
              <div key={section.key} className={`rounded-3xl border shadow-sm p-6 ${card}`}>
                <h2 className={`text-sm font-black mb-4 flex items-center gap-2 ${dark ? 'text-purple-50' : 'text-gray-900'}`}>
                  <RiHandHeartLine size={16} style={{ color: section.color }} />{section.label}
                </h2>
                <div className="flex flex-wrap gap-2 mb-4 min-h-[32px]">
                  ...
                  {displayUser[section.key === 'offered' ? 'offeredSkills' : 'wantedSkills'].length === 0 && (
                    <p className={`text-xs italic ${dark ? 'text-purple-300/30' : 'text-gray-400'}`}>No skills added yet</p>
                  )}
                </div>
                {editing && (
                  <div className="flex gap-2">
                    <input
                      className={`flex-1 text-sm px-3 py-2 rounded-xl border outline-none ${dark ? 'bg-white/5 text-purple-200 border-white/10' : 'border-gray-200 text-gray-700'}`}
                      placeholder={section.placeholder} value={newSkill[section.key]}
                      ...
                      onBlur={e => e.target.style.borderColor = dark ? 'rgba(255,255,255,0.1)' : '#e5e7eb'}
                    />
                    <button onClick={() => addSkill(section.key)} className="w-9 h-9 rounded-xl flex items-center justify-center text-white hover:opacity-80" style={{ backgroundColor: section.color }}><FiPlus size={16} /></button>
                  </div>
                )}
              </div>
            ))}
            <div className={`rounded-3xl border shadow-sm p-6 ${card}`}>
              ...
              <div className="flex flex-wrap gap-2">
                {user.languages.map(lang => (
                  <span key={lang} className="text-xs font-semibold px-3 py-1.5 rounded-full border hover:scale-105 transition-transform cursor-default"
                    style={{ color: '#FF6B35', borderColor: '#FF6B3540', backgroundColor: '#FF6B350d' }}>{lang}</span>
                ))}
              </div>
            </div>
          </div>
