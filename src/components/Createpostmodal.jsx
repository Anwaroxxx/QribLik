...
function ConfirmDelete({ onConfirm, onCancel, dark }) {
  return (
    <div className="flex flex-col items-center gap-4 py-4 px-2 text-center">
      ...
      <div>
        <p className={`font-black text-lg ${dark ? 'text-purple-50' : 'text-gray-900'}`}>
          Delete this post?
        </p>
        <p className={`text-sm mt-1 leading-relaxed ${dark ? 'text-purple-300/50' : 'text-gray-400'}`}>
      ...
      <div className="flex gap-3 w-full mt-1">
        <button onClick={onCancel}
          className={`flex-1 py-2.5 rounded-xl border text-sm font-semibold transition-colors ${
            dark
              ? 'border-white/10 text-purple-300 hover:bg-white/5'
...
export default function CreatePostModal({ onClose, onPost, editPost = null, onDelete = null }) {
  ...
  // shared shorthand
  const modalBg   = dark ? 'bg-[#150d27]' : 'bg-white'
  const inputBg   = dark ? 'bg-white/5 text-purple-100 placeholder-purple-300/30' : 'bg-gray-50 text-gray-800 placeholder-gray-400'
  const labelMuted = dark ? 'text-purple-300/50' : 'text-gray-400'
  ...
  if (published) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center"
        ...
        <div className={`${modalBg} rounded-3xl w-full max-w-xs mx-4 p-8 flex flex-col items-center gap-4 text-center`}
          style={{ boxShadow: dark ? '0 25px 80px rgba(0,0,0,0.6)' : '0 25px 80px rgba(139,63,222,0.25)' }}>
          <div className="w-16 h-16 rounded-2xl flex items-center justify-center"
            style={{ background: 'var(--gradient-qriblik)' }}>
            <FiCheck size={28} className="text-white" strokeWidth={3} />
          </div>
          <div>
            <p className={`font-black text-xl ${dark ? 'text-purple-50' : 'text-gray-900'}`}>
              {isEditing ? 'Post updated!' : 'Post published!'}
            </p>
            <p className={`text-sm mt-1 ${labelMuted}`}>Your neighbors can now see it in the feed.</p>
          </div>
          <div className={`w-full h-1.5 rounded-full overflow-hidden mt-1 ${dark ? 'bg-white/8' : 'bg-gray-100'}`}>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center"
      ...
      onClick={onClose}>
      <div
        className={`${modalBg} rounded-3xl w-full max-w-lg mx-4 overflow-hidden transition-colors duration-500`}
        style={{ boxShadow: dark ? '0 25px 80px rgba(0,0,0,0.6)' : '0 25px 80px rgba(139,63,222,0.2)' }}
      ...
      >
          ...
          <div className="flex items-center gap-2">
            {step === 2 && !isEditing && (
              <button onClick={() => setStep(1)}
                className={`p-1.5 rounded-xl transition-colors ${dark ? 'text-purple-300/50 hover:text-purple-200 hover:bg-white/5' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}>
                <FiArrowLeft size={17} />
              </button>
            )}
            <div>
              <h2 className={`text-xl font-black flex items-center gap-2 ${dark ? 'text-purple-50' : 'text-gray-900'}`}>
                {isEditing && <FiEdit3 size={17} style={{ color: '#8B3FDE' }} />}
                {isEditing ? 'Edit Post' : 'Post Something'}
              </h2>
              <p className={`text-xs mt-0.5 ${labelMuted}`}>
                {isEditing
          ...
          <div className="flex items-center gap-1">
            {isEditing && (
              <button onClick={() => setShowDelete(true)}
                className={`p-2 rounded-xl transition-colors ${dark ? 'text-purple-300/40 hover:text-red-400 hover:bg-red-500/10' : 'text-gray-400 hover:text-red-500 hover:bg-red-50'}`}
                title="Delete post">
                <FiTrash2 size={18} />
              </button>
            )}
            <button onClick={onClose}
              className={`p-2 rounded-xl transition-colors ${dark ? 'text-purple-300/40 hover:text-purple-200 hover:bg-white/5' : 'text-gray-400 hover:text-gray-700 hover:bg-gray-100'}`}>
              <FiX size={20} />
            </button>
          </div>
        </div>
        ...
        <div className="h-[1.5px] mx-6 rounded-full mt-4 mb-1"
          style={{ background: 'var(--gradient-qriblik)', opacity: dark ? 0.25 : 0.15 }} />
        ...
        {!showDelete && step === 1 && (
          <div className="px-6 pb-6 pt-5">
            <p className={`text-[10px] font-bold uppercase tracking-widest mb-3 ${labelMuted}`}>
              ...
              {CATEGORIES.map(cat => {
                ...
                return (
                  <button key={cat.label}
                    ...
                    style={{
                      borderColor: active ? cat.color : (dark ? 'rgba(255,255,255,0.07)' : '#f0f0f0'),
                      backgroundColor: active ? `${cat.color}0d` : (dark ? 'rgba(255,255,255,0.03)' : '#fafafa'),
                      color: active ? cat.color : (dark ? 'rgba(167,139,250,0.45)' : '#6b7280'),
                    ...
                    onMouseLeave={e => {
                      if (!active) {
                        e.currentTarget.style.borderColor = dark ? 'rgba(255,255,255,0.07)' : '#f0f0f0'
                        e.currentTarget.style.backgroundColor = dark ? 'rgba(255,255,255,0.03)' : '#fafafa'
                      }
                    }}
                  >
            ...
            <button
              ...
              style={{
                background: canProceed ? 'var(--gradient-qriblik)' : undefined,
                backgroundColor: !canProceed ? (dark ? 'rgba(255,255,255,0.06)' : '#e5e7eb') : undefined,
                color: canProceed ? 'white' : (dark ? 'rgba(167,139,250,0.3)' : '#9ca3af'),
          ...
          <div className="px-6 pb-6 pt-4 flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className={`text-sm font-semibold ${dark ? 'text-purple-50' : 'text-gray-800'}`}>
                    {currentUser.name}
                  </p>
                  <div className={`flex items-center gap-1 text-xs ${labelMuted}`}>
            ...
            <div className="relative">
              <input
                ...
                maxLength={80}
                className={`w-full px-4 py-3 pr-12 rounded-2xl border text-sm outline-none transition-all duration-200 font-semibold ${inputBg}`}
                style={{ borderColor: title.length > 0 ? (selectedCategory?.color + '60') : (dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb') }}
                onFocus={e => e.target.style.borderColor = selectedCategory?.color || '#8B3FDE'}
                onBlur={e => e.target.style.borderColor = title.length > 0 ? (selectedCategory?.color + '60') : (dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb')}
              />
            ...
            <div className="relative">
              <textarea
                ...
                rows={4}
                maxLength={300}
                className={`w-full px-4 py-3 pb-8 rounded-2xl border text-sm outline-none transition-all duration-200 resize-none leading-relaxed ${inputBg}`}
                style={{ borderColor: description.length > 0 ? (selectedCategory?.color + '60') : (dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb') }}
                onFocus={e => e.target.style.borderColor = selectedCategory?.color || '#8B3FDE'}
                onBlur={e => e.target.style.borderColor = description.length > 0 ? (selectedCategory?.color + '60') : (dark ? 'rgba(255,255,255,0.08)' : '#e5e7eb')}
              />
            ...
            {/* Image preview */}
            {imagePreview && (
              <div className={`relative rounded-2xl overflow-hidden border ${dark ? 'border-white/8' : 'border-gray-100'}`}>
            ...
            {/* Actions row */}
            <div className={`flex items-center justify-between pt-1 border-t ${dark ? 'border-white/6' : 'border-gray-50'}`}>
              <button onClick={() => fileRef.current.click()}
                className={`flex items-center gap-1.5 text-sm px-3 py-2 rounded-xl transition-all ${dark ? 'hover:bg-white/5' : 'hover:bg-gray-50'}`}
                style={{ color: imagePreview ? (selectedCategory?.color || '#8B3FDE') : (dark ? 'rgba(167,139,250,0.35)' : '#9ca3af') }}>
                <FiImage size={16} />
              ...
              <button onClick={handleSubmit} disabled={!canSubmit}
                ...
                style={{
                  background: canSubmit ? 'var(--gradient-qriblik)' : undefined,
                  backgroundColor: !canSubmit ? (dark ? 'rgba(255,255,255,0.06)' : '#e5e7eb') : undefined,
                  color: canSubmit ? 'white' : (dark ? 'rgba(167,139,250,0.3)' : '#9ca3af'),
