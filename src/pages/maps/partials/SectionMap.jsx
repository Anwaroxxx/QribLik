...
// ── Mobile bottom sheet ───────────────────────────────────────────────────
function MobileUserSheet({ user, dark, onAccept, onSkip, onClose, imgUrl }) {
        ...
        <div style={{ padding: "8px 20px 40px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              {user.verified && (
                <span style={{ fontSize: 10, background: "#22C55E15", color: "#22C55E", padding: "3px 10px", borderRadius: 20, fontWeight: 700, border: "1px solid #22C55E25" }}>✓ Verified</span>
              )}
              <button onClick={onClose} style={{ border:"none", background:"transparent", cursor:"pointer", fontSize:24, color:mute, lineHeight:1, padding:4, WebkitTapHighlightColor:"transparent" }}>✕</button>
            </div>
          </div>

          {/* Profile */}
          <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
            <div style={{ width:68, height:68, borderRadius:"50%", flexShrink:0, border:`3px solid ${c}`, overflow:"hidden", background:`${c}15`, display:"flex", alignItems:"center", justifyContent:"center", boxShadow:`0 0 24px ${c}44` }}>
              {imgUrl
          ...
          <div style={{ display:"flex", gap:8, marginBottom:14 }}>
            {[
              ...
              <div key={l} style={{ flex:1, textAlign:"center", padding:"8px 4px", background: dark?"rgba(255,255,255,0.04)":`${c}08`, borderRadius:12, border:`1px solid ${dark?"rgba(139,92,246,0.14)":"rgba(139,92,246,0.09)"}` }}>
          ...
          {[
            ...
            <div key={label} style={{ marginBottom:12 }}>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {list.slice(0,4).map(s => <span key={s} style={{ fontSize:11, padding:"5px 12px", borderRadius:20, background:sb, color:sc, border:sbo }}>{s}</span>)}
              </div>
            </div>
          ))}
...
//   dark      - dark mode flag
//   isMobile  - passed from MapPage (no need to recalculate)
//
// KEY CHANGE: this component no longer uses position:fixed with hardcoded offsets.
// It simply fills its parent container (which MapPage sizes correctly).
export default function SectionMap({ category = "all", dark = false, isMobile = false }) {
    ...
    <div style={{ width: "100%", height: "100%", position: "relative" }}>
      ...
      {isMobile && mobileSelected && (
        connected.has(mobileSelected.id) ? (
          <>
            <div onClick={() => setMobileSelected(null)} style={{ position:"fixed", inset:0, zIndex:4000, background:"rgba(0,0,0,0.35)", backdropFilter:"blur(3px)" }} />
            <div style={{
              position:"fixed", bottom:0, left:0, right:0, zIndex:4001,
              background:dark?"#0e0720":"#fff", borderRadius:"24px 24px 0 0",
